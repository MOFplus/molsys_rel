...
int mfp5_open(struct mfp5_file** _file, const char *filename, int can_write){
    ...
    // set mfp5 natoms attribute
    hid_t def_xyz_dset = H5Dopen2(file->file_id, "/default/restart/xyz" ,H5P_DEFAULT); //get the 
    // three lines below taken from https://stackoverflow.com/questions/15786626/get-the-dimensions-of-a-hdf5-dataset
    hid_t dspace = H5Dget_space(def_xyz_dset); // get the space ??? 
